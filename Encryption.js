...
// convert each char to the hex form
 ...
 function replaceHexValu(str,matrix,messageValue)
 { 
   ...
   let NewValue=''
    str= str.split('')
     str= str.forEach((char) => {
       shuffelMatrix(matrix,messageValue)
    
     NewValue+= matrix[char.charCodeAt(0)]
        });
        return NewValue
 }
  // mix the chars in the message.
...
export function Encryption(message) { 
   ...
   { throw new TypeError('not valid argument') }
    ...
    message= message.replaceAll( ' ','//').split('') // replace the space with //
    
    const messageValue = countStringValue(message)  // count the sum of the chars(according to its number in unicode table) in the message 
     const hexValues= converToHex(message) // convert to hex value
     message=''
     hexValues.forEach( element =>
        {
           
           message +=  replaceHexValu (element,matrix,messageValue) // replace each char in the message with elment in coodingmatrix array.
        
        })
     
        message += CodingMatrix[messageValue%175] // add the message value to the message
     message = mixmessage (message)
